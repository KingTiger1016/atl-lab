...
function tableinit(data,columns){
	...
	if(columns.length==5){
		data.forEach((i,j)=>{
			...
			data[j]["content"] = i.lastOwner.split("/")[0].split("=")[1];
		})
	}
	data.forEach((i,j)=>{
...
function callback(name, jdata){
	...
	case "byprojectno":
	      ...
	      break;
	  case "byowner":
		  tableinit(jdata,[columns.ck,columns.owner,columns.SampleNo,columns.SampleName,columns.ProjectNo,columns.CustName,columns.Sales,columns.sampleLocation,columns.PM,columns.insertDate,columns.memo,columns.open])
	      break;
	  case "byalldocs":
		  tableinit(jdata,[columns.ck,columns.ProjectNo,columns.SampleNo,columns.SampleName,columns.CustName,columns.Sales,columns.sampleLocation,columns.PM,columns.insertDate,columns.owner,columns.memo,columns.open])
		  break;
	  ...
	  case "byclosed":
		  ...
		  break;
	  case "checklist":
		  tableinit(jdata,[columns.ck,columns.content,columns.Sales,columns.ProjectNo,columns.sign])
		  break;
	  case "sampleSignIn":
